...
class account_fstr_wizard(osv.osv_memory):
    ...
    _name = 'account_fstr_tax_code.wizard'
    ...
    _columns = {
        ...
        'root_node': fields.many2one('account_fstr_tax_code.category', 'Root node', required=True,),
    ...
    def open_window(self, cr, uid, ids, context=None):
        ...
        result['period_id'] = []
        if data['period_id']:
            result['period_id'] = data['period_id'].id
        result['context'] = str({'fiscalyear': data['fiscalyear'] and data['fiscalyear'][0] or False, 'period_id': result['period_id'], \
    ...
    def print_template(self, cr, uid, ids, context={}):
        ...
        datas = {'period_id': [], 'ids': ids}
        if data['period_from'] and data['period_to']:
            if data['period_from'] == data['period_to']:
                context['periods'] = [data['period_from'][0]]
            else:                 
                context['periods'] = period_obj.build_ctx_periods(cr, uid, data['period_from'][0], data['period_to'][0])            
        else:
            context['period_id'] = False
            context['periods'] = False
        datas['context'] = str({'fiscalyear': data['fiscalyear'] and data['fiscalyear'][0] or False, 'periods': context['periods'], \
                                    'state': data['target_move']})
        ...
        context['fiscalyear_id'] = data_obj.fiscalyear and data_obj.fiscalyear.id or False        
                        
        context['account_fstr_root_node'] = data['root_node'][0]
        context['hide_zero'] = data['hide_zero']        
        context['state'] = data['target_move']
        
        
        return {
            'type': 'ir.actions.report.xml',
            'report_name': 'account_fstr_tax_code.report',
            'datas': datas,
            'context': context,
        }
