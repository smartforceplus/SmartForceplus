...
from openerp.addons.tag_au_accounting.account_fstr.reports.sm_kit import groupe_digits
...
class account_fstr_report(report_sxw.rml_parse, common_report_header):
    ...
    def __init__(self, cr, uid, name, context):
        ...
        try:
            ...
            ept_test_obj = self.pool.get('account_fstr.category').browse(cr, uid, [self.root_node_obj.id], context=context)
#            print str(self.root_node_obj.id)
            for o in ept_test_obj:
                digit_rounding = o.digits_round
            ...
            self.localcontext.update({
                ...
                'date_end': '',
                'digits_round': '0'*(digit_rounding - 2)
            })
        except:
    ...
    #        print self._get_root_id().indent_title
            result = [self._get_statement(cr, uid, ids, [], category_obj, -1, context=context), category_obj.balance]
            result = self._digits_rounding(cr, uid, ids, result, category_obj, context=context)
            return {
                'name': name,
                'statements': result,
                'lang': context['lang'],
    ...
    def _digits_rounding(self, cr, uid, ids, statements, category_obj, context={}):
        digits_round = category_obj.digits_round
...
#                print account_statement_obj.balance
...
report_sxw.report_sxw('report.Account HTML Report', 'account_fstr.category',
                      'addons/accounts_reports_ept/reports/account_fstr_test.mako', parser=account_fstr_report, header="True")
