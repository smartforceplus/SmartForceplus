...
class html__tag_gen_tran(models.TransientModel):

    _name = "html.tag.trans"
    ...
    def setup_form(self):
        ...
        #create tag
        my_tags = self.env['res.partner.category'].search([('name','=', 'Campaign Tag (' + self.campaign_id.name + ')')])
        if len(my_tags) == 0:
        ...
        #create filter
        my_filters = self.env['ir.filters'].search([('name', '=', self.campaign_id.name + ' Campaign Filter')])
        if len(my_filters) == 0:
            domain_string = "[['category_id', 'ilike', '" + my_tag_id.name + "']]"
            my_filter_id = self.env['ir.filters'].create({'name': self.campaign_id.name + ' Campaign Filter', 'model_id':'res.partner', 'domain':domain_string})
        else:
            domain_string = "[['category_id', 'ilike', '" + my_tag_id.name + "']]"
	    my_filter_id = my_filters[0]
        
        
        
        #create segment
        my_segs = self.env['marketing.campaign.segment'].search([('name','=',self.campaign_id.name)])
        if len(my_segs) == 0:
        ...
        _logger.error(str(my_tag_id.id))

        #create form
        new_form_id = self.env['html.tag.formgen'].create({'name': self.name,'campaign_id':self.campaign_id.id, 'tag_id':my_tag_id.id})
        ...
        self._cr.execute('INSERT INTO html_tag_formgen_ir_model_fields_rel VALUES(%s,%s)',(new_form_id.id,contact_name.id,))
        self._cr.execute('INSERT INTO html_tag_formgen_ir_model_fields_rel VALUES(%s,%s)',(new_form_id.id,email_from.id,))
        
        return {
	                    'name': 'HTML Forms',
	                    'view_type': 'form',
	                    'view_mode': 'form',
	                    'res_model': 'html.tag.formgen',
	                    'type': 'ir.actions.act_window',
	                    'res_id': new_form_id.id or False,
               }
class html_tag_gen(models.Model):

    _name = "html.tag.formgen"
    ...
    def generate_form_odoo(self):
        ...
        html_output += '<form method="POST" class="form-horizontal mt32" action="' + request.httprequest.host_url + 'form/tag/myinsert">' + "\n"
    ...
    def generate_form_optimize(self):
        html_output = ""
        html_output += '<div id="contact_body">' + "\n"
        html_output += '<form method="POST" action="' + request.httprequest.host_url + 'form/tag/myinsert">' + "\n"
        html_output += '<input type="hidden" name="form_id" value="' + str(self.id) + '"' + "\n"
        for ff in self.fields_ids:
